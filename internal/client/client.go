...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "mymind")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// not during dry-run.
	authHeader, err := c.authHeader(method, path)
	if err != nil {
		return nil, 0, err
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "mymind/0.4.0-alpha")
...
func (c *Client) authHeader(method, path string) (string, error) {
	if c.Config == nil {
		return "", nil
	}
	...
	if c.Config.MymindKid != "" && c.Config.MymindSecret != "" {
		token, err := signMyMindRequest(c.Config.MymindKid, c.Config.MymindSecret, method, path)
		if err != nil {
			return "", err
		}
		c.Config.AuthSource = "kid_secret"
		return "Bearer " + token, nil
	}
	return c.Config.AuthHeader(), nil
}

func signMyMindRequest(kid, secret, method, path string) (string, error) {
	secretBytes, err := decodeBase64Secret(secret)
	if err != nil {
		return "", err
	}
	now := time.Now().Unix()
	header := map[string]any{"alg": "HS256", "kid": kid}
	...
	encodedPayload, err := base64URLJSON(payload)
	if err != nil {
		return "", err
	}
...
func decodeBase64Secret(secret string) ([]byte, error) {
	...
	return nil, fmt.Errorf("MYMIND_SECRET must be valid base64")
}
