...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MYMIND_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MYMIND_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mymind/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.mymind/feedback.jsonl.
When ` + "`MYMIND_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MYMIND_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mymind feedback list
  mymind feedback list --limit 5
  mymind feedback list --json`,
