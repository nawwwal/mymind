...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Mymind",
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set your access key:")
				fmt.Fprintln(w, "  export MYMIND_KID=\"your-kid\"")
				fmt.Fprintln(w, "  export MYMIND_SECRET=\"your-base64-secret\"")
				fmt.Fprintf(w, "  mymind auth set-key <kid> <base64-secret>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetKeyCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:     "set-key <kid> <base64-secret>",
		Short:   "Save a mymind access key to the config file",
		Example: "  mymind auth set-key YOUR_KID YOUR_BASE64_SECRET",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg.AccessToken = ""
			cfg.MymindSignedJwt = ""
			cfg.MymindKid = args[0]
			cfg.MymindSecret = args[1]
			if err := cfg.SaveTokens("", "", "", "", cfg.TokenExpiry); err != nil {
				return configErr(fmt.Errorf("saving key: %w", err))
			}
			...
			fmt.Fprintf(cmd.OutOrStdout(), "Access key saved to %s\n", cfg.Path)
			return nil
		},
	}
}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mymind auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("MYMIND_SIGNED_JWT") != "" {
				envStillSet = "MYMIND_SIGNED_JWT"
			}
