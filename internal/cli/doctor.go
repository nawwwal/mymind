...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mymind doctor
  mymind doctor --json
  mymind doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MYMIND_KID=<kid> MYMIND_SECRET=<base64-secret>"
				} else {
			...
			if os.Getenv("MYMIND_KID") != "" && os.Getenv("MYMIND_SECRET") != "" {
				authEnvSet = append(authEnvSet, "MYMIND_KID", "MYMIND_SECRET")
			} else if os.Getenv("MYMIND_SIGNED_JWT") != "" {
				authEnvSet = append(authEnvSet, "MYMIND_SIGNED_JWT")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MYMIND_KID and MYMIND_SECRET")
			}
			switch {
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d auth mode available", len(authEnvSet))
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						_, authErr := c.GetWithHeaders(verifyPath, authParams, nil)
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'mymind sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'mymind sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'mymind sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'mymind sync' to refresh."
	}
	return report
}
