...
Exit codes & warnings:
  ...
  mymind sync --resources channels,messages
  ...
  mymind sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  mymind sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "mymind_search":
		return db.UpsertMymindSearch(data)
	case "objects":
		return db.UpsertObjects(data)
	case "blob":
		return db.UpsertBlob(data)
	case "content":
	...
	case "thumbnail":
		return db.UpsertThumbnail(data)
	default:
